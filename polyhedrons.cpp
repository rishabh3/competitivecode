...
void testcase() {
	...
	for(int i = 0;i < n;i++) {
		cin >> temp;
		tot += shape_to_num.find(temp)->second;
	}
	cout << tot << endl;
}
...
int main() {
	shape_to_num.clear();
	shape_to_num["Tetrahedron"] = 4;
	shape_to_num["Cube"] = 6;
	shape_to_num["Octahedron"] = 8;
	shape_to_num["Dodecahedron"] = 12;
	shape_to_num["Icosahedron"] = 20;
	testcase();
	return 0;
}
